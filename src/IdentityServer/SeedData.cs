...
namespace IdentityServer
{
    ...
    {
        public static void EnsureSeedData(IServiceProvider service)
        {
            Console.WriteLine("Seeding database...");

            using (var scope = service.GetRequiredService<IServiceScopeFactory>().CreateScope())
            {
                var application = scope.ServiceProvider.GetService<ApplicationDbContext>();
                Console.WriteLine("Migrating ApplicationDbContext");
                application.Database.Migrate();
                var grant = scope.ServiceProvider.GetRequiredService<PersistedGrantDbContext>();
                Console.WriteLine("Migrating PersistedGrantDbContext");
                grant.Database.Migrate();
